...
class UnicodeProcessor:
    ...
    def __call__(
        ...
        text_ids = np.zeros((len(text_list), text_ids_lengths.max()), dtype=np.int64) # padded sequence length
...
def load_voice_style(voice_style_paths: list[str], verbose: bool = False) -> Style:
    ...
    ttl_style = np.zeros([bsz, ttl_dims[1], ttl_dims[2]], dtype=np.float32) # B, 50, 256
    dp_style = np.zeros([bsz, dp_dims[1], dp_dims[2]], dtype=np.float32) # B, 8, 16
    ...
    for i, voice_style_path in enumerate(voice_style_paths):
        ...
        ttl_style[i] = ttl_data.reshape(ttl_dims[1], ttl_dims[2]) # (50, 256)

        dp_data = np.array(voice_style["style_dp"]["data"], dtype=np.float32).flatten()
        dp_style[i] = dp_data.reshape(dp_dims[1], dp_dims[2]) # (8, 16)
